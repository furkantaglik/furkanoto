...
import { getSearchResults } from "@/utils/functions/Datafetcher"
...
const SearchBar = () => {
    ...
    return (
        <section className='relative'>
            <input
                value={searchValue}
                onChange={(e) => { setSearchValue(e.target.value), handleResults() }}
                type="text"
                placeholder="Otomotiv Ara."
                className="bg-neutral-800 px-2 py-1 text-white outline-none border-transparent border-b-4 focus:border-blue-500"
            />
                        ...
                        <Link key={index}
                            ...
                            <li className="p-1 flex text-white bg-gradient-to-r from-black to-slate-950 hover:bg-gradient-to-r hover:from-black hover:to-blue-950 w-[400px] items-center">
                                ...
                                <div className="grid ms-5 gap-y-2">
                                    <h2 className="font-bold">{result.brand} {result.model}</h2>
                                    <div className="flex gap-x-3 text-gray-600 text-sm">
                                        <p>{result.fuel}</p>
                                        <p>{result.gear}</p>
                                        <p>{result.year}</p>
                                    </div>
